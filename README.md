...
Nos próximos três artigos da série, a que damos início, intentamos que os roteiros obedeçam à geometria das ruas, seguindo exatamente o traçado das vias. Para isso, exploramos inicialmente, as funcionalidades do pacote **Osmnx** em conjunto com **NetworkX** , depois a **API** do **OpenRouteService(ORS)** e finalmente a solução de roteamento **Open Source Routing Machine(OSRM)**.
